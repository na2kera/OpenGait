# =====================================================
# Multiple FID Evaluation Script for CASIA-B (per subject)
# =====================================================
...
def collect_image_paths_by_subject_and_condition(root):
    """
    CASIA-B構造から、人物×条件（nm/bg/cl）で画像パスを収集
    戻り値: { 'nm': {'001': [...], '002': [...]}, 'bg': {...}, 'cl': {...} }
    """
    data = {'nm': {}, 'bg': {}, 'cl': {}}
    ...
    for sid in subjects:
        ...
        if not os.path.isdir(sub_dir):
            continue
        for cond in os.listdir(sub_dir):  # nm-01, bg-01, cl-01
            cond_type = cond[:2]  # 'nm', 'bg', 'cl'
            if cond_type not in data:
                continue
            cond_dir = os.path.join(sub_dir, cond)
            all_imgs = []
            for view in os.listdir(cond_dir):  # 000,018,...
...
if __name__ == "__main__":
    ...
    for root in dataset_roots:
        ...
        if not os.path.exists(root):
            ...
            overall_results[root] = {"nm": None, "bg": None, "cl": None}
            continue
