...
class DINOv2FeatureExtractor(nn.Module):
    def __init__(self):
        super().__init__()
        # facebookresearch/dinov2 から直接ロード
    ...
    def forward(self, x):
        ...
        if isinstance(feats, dict):
            feats = feats["x_norm_patchtokens"].mean(dim=1)  # 特徴マップ平均（安全策）
        return feats
...
def compute_subject_features(paths, extractor, device):
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # DINOv2入力サイズ
        transforms.ToTensor(),
        transforms.Lambda(lambda x: x.repeat(3, 1, 1)),  # グレースケール→RGB
...
if __name__ == "__main__":
    dataset_roots = [ "/data/CASIA-B-png/nm1-bg1-cl1-png" ]
